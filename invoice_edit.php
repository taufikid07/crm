...
while($tl = $xoopsDB->fetchArray($res_grup)){
	$group_selected = "SELECT * FROM ".$xoopsDB->prefix("crm_invoice")." AS np INNER JOIN ".$xoopsDB->prefix("crm_kontak")." AS k ON np.id_kontak = k.id_kontak WHERE np.id_perusahaan='".$edata['id_perusahaan']."' AND np.id_invoice='".$edata['id_invoice']."' ORDER BY k.nama ASC";	
	$group_s = $xoopsDB->query($group_selected);
	$selected='';
	while($gs = $xoopsDB->fetchArray($group_s)){
...
$pub_kgt='';
while($tl = $xoopsDB->fetchArray($result_group)){
	$group_selected = "SELECT * FROM ".$xoopsDB->prefix("crm_invoice")." AS qt INNER JOIN ".$xoopsDB->prefix("crm_kegiatan")." AS k ON qt.id_kegiatan = k.id_kegiatan WHERE k.id_kegiatan='".$edata['id_kegiatan']."' AND qt.id_invoice='".$edata['id_invoice']."' ORDER BY k.nama_kegiatan ASC";	
	$group_s = $xoopsDB->query($group_selected);
	$selected='';
	while($gs = $xoopsDB->fetchArray($group_s)){
...
// Activity
$act='';
	$qkeg = "SELECT * FROM ".$xoopsDB->prefix("crm_listinvoice")." WHERE id_invoice=".$edata['id_invoice']." ORDER BY orderby ASC";
...
$act.='
	<tr>
		...
		<td>
			...
			<a href="q_list_unit_editquote?pr='.$pr.'&quote='.$quote.'" class="btn btn-primary btn-mini"><i class="icon-cogs"></i> Setting</a>
		</td>
...
while($ttl = $xoopsDB->fetchArray($result_group)){
	...
	$bhsa.=' <option value="'.$ttl['bahasa'].'" '.$selected.'>'.$bahasa.'</option> ';
}
//Pajak
...
while($ttl = $xoopsDB->fetchArray($resultg)){
	$group_selected = "SELECT * FROM ".$xoopsDB->prefix("crm_invoice")." AS qt WHERE qt.id_quote='".$edata['id_invoice']."' AND tax_percent='".$edata['tax_percent']."'";	
	$group_s = $xoopsDB->query($group_selected);
	$selected='';
	while($gs = $xoopsDB->fetchArray($group_s)){
	...
	$pjk.=' <option value="'.$ttl['tax_percent'].'" '.$selected.'>'.$pajak.'</option> ';
}
            ...
            <div class="span12">
                ...
                <div class="widget black">
                    <div class="widget-title">
                        <h4><i class="icon-reorder"></i> Edit invoice </h4>
                        <div class="actions sm-btn-position">
                            <a href="invoice_next?pr='.$pr.'" class="btn btn-primary btn-small"> Back <i class="icon-arrow-right"></i> </a>
                        </div>
                        <div class="tools">
                    ...
                    <div class="widget-body form">
                        ...
                        <div class="widget-body">
                         ...
                         <div class="space15"></div>
                            <form class="cmxform form-horizontal" id="commentForm" name="form_perusahaan" action="model/model_invoice?uid='.$uid.'&pr='.$pr.'&keg='.$keg.'&quote='.$quote.'&inv='.$inv.'" method="post">
                             <div class="row-fluid">
                                 ...
                                 <div class="span8 billing-form">
                                     <div class="space10"></div>
                                         <div class="control-group ">
                                             ...
                                             <input name="no_invoice" type="text" placeholder="" class="span5" value="'.$edata['no_invoice'].'" disabled="disabled">
                                         </div>
                                         <div class="control-group ">
                             ...
                             <div class="row-fluid">
                                 ...
                                 <div class="span12">
                                    <div class="row-fluid text-center">
                                        <button class="btn btn-success" type="submit" name="update">Update invoice</button>
                                     </div>
                                 </div>
                                 </form>
                             </div>
