...
$query=$xoopsDB->query("SELECT * FROM ".$xoopsDB->prefix("crm_invoice")." AS qt where id_invoice =".$inv."");
$edata = $xoopsDB->fetchArray($query);
//Perusahaan
$qpr=$xoopsDB->query("SELECT * FROM ".$xoopsDB->prefix("crm_invoice")." AS qt 
INNER JOIN ".$xoopsDB->prefix("crm_perusahaan")." AS p ON qt.id_perusahaan = p.id_perusahaan 
WHERE qt.id_perusahaan='".$edata['id_perusahaan']."' AND qt.id_invoice='".$edata['id_invoice']."'");
...
//Client
$qcl=$xoopsDB->query("SELECT * FROM ".$xoopsDB->prefix("crm_invoice")." AS np INNER JOIN ".$xoopsDB->prefix("crm_kontak")." AS k ON np.id_kontak = k.id_kontak WHERE np.id_kontak='".$edata['id_kontak']."' AND np.id_invoice='".$edata['id_invoice']."' ORDER BY k.nama ASC");
$dcl = $xoopsDB->fetchArray($qcl);
//Grup
...
$matu=$edata['bahasa'];
$listpro='';
$qlistk = "SELECT * FROM ".$xoopsDB->prefix("crm_listinvoice")." WHERE id_invoice=".$edata['id_invoice']." ORDER BY orderby ASC";
...
while($tl = $xoopsDB->fetchArray($result_group)){
	$group_selected = "SELECT * FROM ".$xoopsDB->prefix("crm_invoice")." AS qt WHERE qt.id_invoice='".$edata['id_invoice']."' AND qt.kop_stat='".$edata['kopstat']."'";	
...
while($ttl = $xoopsDB->fetchArray($result_group)){
	$group_selected = "SELECT * FROM ".$xoopsDB->prefix("crm_invoice")." AS qt WHERE qt.id_invoice='".$edata['id_invoice']."' AND qt.ttdstat='".$edata['ttdstat']."'";	
	$group_s = $xoopsDB->query($group_selected);
	$selected='';
	while($gs = $xoopsDB->fetchArray($group_s)){
...
if ($edata['status'] == '0' && $uid == $edata['project_manager']){
	$submit.='
	<form name="form_perusahaan" action="model/model_invoice?uid='.$uid.'&pr='.$pr.'&keg='.$keg.'&quote='.$quote.'&inv='.$inv.'" class="form-horizontal" method="post">
	<button class="btn btn-success btn-large" type="submit" name="checked"><i class="icon-check"></i>Check</button>
	<a href="invoice_edit?uid='.$uid.'&pr='.$pr.'&keg='.$keg.'&quote='.$quote.'&inv='.$inv.'" class="btn btn-warning btn-large hidden-print"> Edit  <i class="icon-pencil"></i></a>
	<a href="invo" class="btn btn-success btn-large hidden-print"> List <i class="icon-check"></i></a>
	</form>	';
	...
	<div class="alert alert-block alert-warning fade in">
	  ...
	  <p><br>
		  Invoice ini Masih Menunggu Pengecekan dari <b><i>'.$dpm['name'].'</i></b> <br>Terima kasih.
	  </p>
	</div>
	<a href="invo" class="btn btn-success btn-large hidden-print"> List <i class="icon-check"></i></a>';
}else if ($edata['status'] == '1' && $uid=='10' || $edata['status'] == '1' && $uid=='154' || $edata['status'] == '1' && $uid=='131'){
	$submit.='
	<form name="form_perusahaan" action="model/model_invoice?uid='.$uid.'&pr='.$pr.'&keg='.$keg.'&quote='.$quote.'&inv='.$inv.'" class="form-horizontal" method="post">
	<button class="btn btn-success btn-large" type="submit" name="approve"><i class="icon-check"></i>Approve</button>
	<form>
	<a href="invoice_edit?pr='.$pr.'&keg='.$keg.'&quote='.$quote.'&inv='.$inv.'" class="btn btn-warning btn-large hidden-print"> Edit  <i class="icon-pencil"></i></a>
	<a href="invo" class="btn btn-success btn-large hidden-print"> List <i class="icon-check"></i></a>';
	...
	<div class="alert alert-block alert-info fade in">
	  <h4 class="alert-heading"><u>'.$duser['name'].'</u></h4>
	  <p><br>
		  Invoice ini Masih Menunggu Approvel dari Direksi atau Finance. <br>Terima kasih.
	  </p>
	</div>
	<a href="invo" class="btn btn-success btn-large hidden-print"> List <i class="icon-check"></i></a>';
	...
	<div class="alert alert-block alert-success fade in">
	  <h4 class="alert-heading"><u>'.$duser['name'].'</u></h4>
	  <p><br>
		  Invoice ini sudah berhasil diapprove. <br>Apabila ada kesalahan sistem silahkan hubungi tim IT. Terima kasih.
	  </p>
	</div>
	<a href="invo" class="btn btn-success btn-large hidden-print"> List  <i class="icon-check"></i></a>';
}
...
if (isset($_GET['add_reply'])){
	...
	$qrep ="INSERT INTO ".$xoopsDB->prefix("crm_replyin")." VALUES ('', '$koment', '$uid', '$nowr', '$reply', '$rmail')";
	$xoopsDB->queryF($qrep);
	$irep=$xoopsDB->getInsertId();
	$qquote=$xoopsDB->query("SELECT * FROM ".$xoopsDB->prefix("crm_invoice")." AS q
								INNER JOIN ".$xoopsDB->prefix("crm_komentarin")." AS k ON k.id_invoice = q.id_invoice
								INNER JOIN ".$xoopsDB->prefix("crm_replyin")." AS r ON r.id_komentar = k.id_komentar
								WHERE q.id_invoice = ".$inv." AND k.id_komentar = ".$koment." AND r.id_reply = ".$irep);
	$dquote = $xoopsDB->fetchArray($qquote);	
	$qkm=$xoopsDB->query("SELECT * FROM ".$xoopsDB->prefix("crm_komentarin")." WHERE id_komentarin=".$koment);
	...
	$mail->FromName = "No Invoice ".$dquote['no_invoice']." Post Reply";
	...
	$mail->AddAttachment("/tmp/image.jpg", "new.jpg"); // attachment
		...
		$mail->Subject = "No Invoice ".$dquote['no_invoice']." - Balas ";
		$mail->Body = '		
					...
					<td style="width:100%; background-color:#fff; margin:auto; border-radius: 10px; padding:5px;">
						...
						<table style="width:100%;">
						  <tr>
							...
							Post Reply oleh '.$dpos['name'].' pada kode invoice '.$dquote['no_invoice'].'.
							</td>
						  </tr>
						  <tr style="background:#eeeeee; padding:30px;">
								<td width="200" style="background:#eeeeee; font-weight:800; padding:15px;">Kode invoice</td>
						  ...
						  <tr>
							...
							<center>
							<a target="_blank" href="'.XOOPS_URL.'/modules/crm/admin/view_invoice?pr='.$pr.'&keg='.$keg.'&quote='.$quote.'&inv='.$inv.'" style="background-color: #4CAF50; border: none; color: white; padding: 15px 32px; text-align: center; text-decoration: none; display: inline-block; font-size: 16px;margin: 4px 2px; cursor: pointer;"> 
	...
	header('Location:view_invoice?pr='.$pr.'&keg='.$keg.'&quote='.$quote.'&inv='.$inv);
}
...
if (isset($_GET['komen'])){
	...
	$invoicek	= $_GET['invoice'];
	$pr			= $_GET['pr'];
	$now=date("Y-m-d H:i:s");
	$qcom ="INSERT INTO ".$xoopsDB->prefix("crm_komentarin")." VALUES ('', '$invoicek', '$uid', '$now', '$komentar', '$emailto')";
	$xoopsDB->queryF($qcom);
	
	$ikom=$xoopsDB->getInsertId();
	$qquote=$xoopsDB->query("SELECT * FROM ".$xoopsDB->prefix("crm_invoice")." AS q
								INNER JOIN ".$xoopsDB->prefix("crm_komentarin")." AS k ON k.id_quote = q.id_quote
								WHERE q.id_invoice = ".$invoicek." AND k.id_komentar = ".$ikom);
	$dquote = $xoopsDB->fetchArray($qquote);	
	$qpos=$xoopsDB->query("SELECT * FROM ".$xoopsDB->prefix("users")." WHERE uid=".$uid);
	$dpos = $xoopsDB->fetchArray($qpos);
	...
	$mail->From = $webmaster_email;
	$mail->FromName = "No Invoice ".$dquote['no_invoice']." Post Comments";
	$mail->AddAddress($email,$name);
	//$mail->AddAddress('taufikid07@gmail.com','Finance');
	$mail->AddReplyTo($webmaster_email,"namawebmaster");
	$mail->WordWrap = 50; // set word wrap
	...
	$mail->AddAttachment("/tmp/image.jpg", "new.jpg"); // attachment
		$mail->IsHTML(true); // send as HTML
		$mail->Subject = "No Invoice ".$dquote['no_invoice']." - Komentar";
		$mail->Body = '		
		<table style="width:100%; margin:auto; background: #ccc; background: -moz-linear-gradient( center top, #ccc 30%, #fff 100% ); background: -webkit-gradient( linear, left top, left bottom, color-stop(.2, #ccc), color-stop(1, #fff) );">
		  <tr>
			<td>
...
$komentar='';
$qkomen ="SELECT * FROM ".$xoopsDB->prefix("crm_komentarin")." WHERE id_invoice='".$edata['id_invoice']."'";
...
while ($dkomen = $xoopsDB->fetchArray($rkomen)){
	...
	<div class="media-body">
		 ...
		 $qreply ="SELECT * FROM ".$xoopsDB->prefix("crm_replyin")." WHERE id_komentar='".$dkomen['id_komentar']."'";
				...
				<div class="span12">
					...
					<div class="widget black">
						...
						<div class="widget-body form">
							<form class="cmxform form-horizontal" id="commentForm" action="view_invoice?pr='.$pr.'&keg='.$keg.'&quote='.$quote.'&inv='.$inv.'" method="GET">
...
<div class="post-comment">
	<h3>Post Comments</h3>
	<form action="view_invoice?pr='.$pr.'&keg='.$keg.'&quote='.$quote.'&inv='.$inv.'" method="GET">
	 <label>Message</label>
...
// HISTORY
$qhstr = " SELECT * FROM ".$xoopsDB->prefix("crm_history")." AS h WHERE h.id_quote=".$quote." AND h.id_invoice=".$inv." ORDER BY h.id_history DESC limit 10	";
	 ...
	 <div class="msg-time-chat">
		 ...
		 <div class="'.$zebra.'">
			 ...
			 <div class="text">
				 ...
				 <p>'.$dhst['komentar'].' <a href="#">'.$dquote['no_invoice'].'</a></p>
			 </div>
		 </div>
	 </div>
	 <!-- /comment -->
            ...
            <!-- BEGIN PAGE HEADER--> 
                                 ...
                                 <div class="row-fluid">
                                    <div class="widget-body">
                                     <div class="span5 pull-left">
                                        ...
                                        <div class="widget-body">
                                            <!-- BEGIN FORM-->
                                            <form action="model/model_invoice?uid='.$uid.'&pr='.$pr.'&keg='.$keg.'&quote='.$quote.'&inv='.$inv.'" class="form-horizontal" method="post">
                                                <div class="control-group">
                                                    ...
                                                    <div class="form-actions">
                                                        <button type="submit" class="btn blue" name="kops_invoice"><i class="icon-ok"></i> Save</button>
                                                    </div>
                                            </form>
                                            <!-- END FORM-->
                                        </div>
                ...
                <div class="row-fluid">
                     ...
                     <div class="widget red">
                         <div class="widget-title">
                             <h4><i id="dash-timeline-icon" class="icon-spinner icon-spin"></i> History for Quotation '.$edata['no_invoice'].'</h4>
                                    <span class="tools">
                                    <a href="javascript:;" class="icon-chevron-down"></a>
                                    <a href="javascript:;" class="icon-remove"></a>
                                    </span>
