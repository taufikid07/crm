...
$qpr = "SELECT p.`status`, p.fax, p.kontak_perusahaan, p.nama, us.`name`, kg.nama_kegiatan
FROM ".$xoopsDB->prefix("crm_invoice")." AS qt 
...
//Client
$qcl = "SELECT k.nama_kontak, k.no_hp, k.email FROM ".$xoopsDB->prefix("crm_invoice")." AS np INNER JOIN ".$xoopsDB->prefix("crm_kontak")." AS k ON np.id_kontak = k.id_kontak WHERE np.id_kontak='".$edata['id_kontak']."' AND np.id_invoice='".$edata['id_invoice']."' ORDER BY k.nama_kontak ASC";
$rescl = $xoopsDB->query($qcl);
$dcl = $xoopsDB->fetchArray($rescl);


//Prepared by :
$qpre = $xoopsDB->query("SELECT * FROM ".$xoopsDB->prefix("users")." AS u WHERE uid='".$uid."'");	
$dpre = $xoopsDB->fetchArray($qpre);

//$matu=$edata['bahasa'];
$listpro='';
$qlistk = "SELECT lp.description,lp.quantity,lp.unit,lp.order,lp.unit_price,lp.total,lp.orderby 
FROM ".$xoopsDB->prefix("crm_listinvoice")." AS lp WHERE id_invoice=".$edata['id_invoice']." ORDER BY orderby ASC";
        ...
        <table class="tabel-foothead">
            <tr>
                <td style="text-align: Center; width: 100%; z-index:1;">
					<img src="img/Invoice_header.png" style="width:725px;">
				</td>
            </tr>			
        </table>
...
<div class="special">
	<table>
		<tr>
			...
			<span style="font-weight:bold;"> INVOICE</span>  # '.$edata['no_invoice'].' </td>
		</tr>
	</table>
</div>
	...
	<tr style="vertical-align: top">
		<td>
			<div class="zone" style="text-align: center; vertical-align: middle;">
				...
				With all due respect, we hereby submit invoice related activities are listed below.
			</div>
		</td>	
	</tr>
</table>
...
try
{
	...
	$html2pdf->Output('Draf_Invoice-'.$datenow2.'-'.$dkp['nama_kegiatan'].'.pdf');
}
